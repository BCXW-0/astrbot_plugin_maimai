...
[![Version](https://img.shields.io/badge/Version-2.1.0-brightgreen.svg)](https://github.com/BCXW-0/astrbot_plugin_maimai)
...
- `2.1.0`：WebUI 自动下载谱面、调用本地模型打标、检索标签，并建立标签条目与谱面文件/信息映射
...
用途：锐评人格、自动打标、谱面标签管理和配置摘要；自动打标只在 WebUI 中操作，指令触发方式以 [`static/help.txt`](static/help.txt) 为准。
监听非本机地址时必须配置 Token。
...
- 按 ID、曲名、艺术家、谱师或定数查看模型标签、概率、分析窗口和来源映射。
...
训练元数据直接读取插件内的 `static/Levels`，只分析 `lv_4`、`lv_5`、`lv_6` 中定数不低于 `12.6` 的难度。WebUI 自动打标使用已训练的本地模型，不调用 AstrBot 谱面模型；下载谱面时才访问 OneCat 官谱接口。
