...
- `2.5.0`：优化打标签模型，提高准确率；增加双模型一致与联网来源门禁，并将未被五标签展示槽位截断的 XLS 候选作为弱监督，减少标签缺失；收紧协调/留尾泛化候选，补齐定拍与局部定位候选，运行时优先保留高置信时序标签并避免空结果
...
规则引擎按 XLS 的候选特征、难点特征和同定数占比上限工作，使用连续两小节局部窗口，输出最多 5 个主要标签，并为每个标签保存窗口、事件原始语法或撞尾候选位置。协调要求局部高密度跨区、重复短纵或重复大位移，留尾要求局部高数量的高速短星/复杂 Slide 出张；定拍使用窗口内锚定键，不受全谱节奏干扰。运行时选标签时先保留高置信爆发与星星时序族，再补充节奏和结构配置；`延迟星星` 与 `拆弹` 属于同一显示族，只保留概率较高者；若校准阈值没有命中，则只在本地模型最高概率达到回退门槛时保留一个可审计结果。数据集保留完整 `inote`、事件序列、BPM 变化、定数、文件 SHA-256、排除的 Ex 目标和三类标签位置：强监督五标签抽样、未截断的 XLS 弱候选，以及最终展示标签。五标签上限只约束用户端展示和强监督抽样；被展示槽位截断的候选不会被当成负例。
...
当前训练产物使用 121 条通过双模型并保留可追溯媒体来源的样本，其中 11 条达到严格 80% 重合阈值，其余按重合度置信度降级选择；按歌曲分组划分训练/验证/留出集，并使用 190 维特征、五成员集成、早停、类别不平衡权重、XLS 候选弱监督和逐标签阈值校准。只有验证集 precision ≥ 0.80、留出集 precision ≥ 0.80、留出集 F1 ≥ 0.40 且 recall ≥ 0.25 的模型才会被写入；本次验证集 precision 为 97.90%，留出集 precision 为 95.80%，recall 为 84.31%，F1 为 89.69%，最佳验证 Loss 为 0.35128345。全量运行时验收覆盖 2003 个 12.0+ 难度，空标签和分析错误均为 0；定位候选要求同一两小节窗口内的快速大跨度 Slide 与局部密度/位移条件同时成立，撞尾只有通过本地路径区时序复核并进入模型候选分数时才保留展示名额。
