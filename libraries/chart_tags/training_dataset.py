...
MIN_HOLDOUT_RECALL = 0.25
MIN_CANDIDATE_SUPPORT = 5
MIN_TARGET_POSITIVES = 2
WEAK_CANDIDATE_WEIGHT = 0.35
...
def validate_records(records: list[dict[str, Any]]) -> None:
    ...
    for record in records:
        ...
        if not isinstance(record.get("training_tag_scores"), dict) or not isinstance(record.get("training_tag_sources"), dict):
            raise ValueError(f"记录缺少训练标签置信度或来源: {key}")
        ...
        if not isinstance(record.get("weak_candidate_positions"), dict):
            raise ValueError(f"记录缺少弱候选标签位置: {key}")
...
def _record_features(record: dict[str, Any]) -> dict[str, float]:
    ...
    for tag in filter_allowed_tags(record.get("raw_tags") or []):
        result[f"rule.candidate.{tag}"] = 1.0
    for tag in filter_allowed_tags(record.get("difficulty_tags") or []):
        result[f"rule.difficulty.{tag}"] = 1.0
    for tag, score in (record.get("candidate_scores") or {}).items():
        try:
            numeric_score = float(score)
        except (TypeError, ValueError):
            continue
        if np.isfinite(numeric_score):
            result[f"rule.score.{tag}"] = numeric_score
    for tag, score in (record.get("difficulty_scores") or {}).items():
        try:
            numeric_score = float(score)
        except (TypeError, ValueError):
            continue
        if np.isfinite(numeric_score):
            result[f"rule.difficulty_score.{tag}"] = numeric_score
    source = record.get("source") if isinstance(record.get("source"), dict) else {}
    chart = record.get("chart") if isinstance(record.get("chart"), dict) else {}
    collision = record.get("collision") if isinstance(record.get("collision"), dict) else {}
    result.update({
...
def _training_target_tags(record: dict[str, Any]) -> list[str]:
    return filter_allowed_tags([
        *(record.get(TRAINING_TARGET_FIELD) or []),
...
def _target_matrix(records: list[dict[str, Any]]) -> np.ndarray:
    """Train on strong labels plus uncapped XLS candidates.

    ``training_tags`` is the five-label, auditable strong-target sample used
    for display and reporting.  A candidate omitted only because that sample
    is full is not a negative example, so raw/difficulty candidates are also
    weak positive targets.  This keeps the model from learning the arbitrary
    five-label packing order while the runtime output remains capped.
    """
    return np.asarray(
        [[
            1.0 if tag in set(_training_target_tags(record)) else 0.0
...
def _supervision_matrix(records: list[dict[str, Any]]) -> np.ndarray:
    ...
    for row, record in enumerate(records):
        ...
        target_tags = set(filter_allowed_tags(record.get(TRAINING_TARGET_FIELD) or []))
        weak_candidates = set(filter_allowed_tags([
            *(record.get("raw_tags") or []),
            *(record.get("difficulty_tags") or []),
        ])) - target_tags
        for tag in weak_candidates:
            matrix[row, positions[tag]] = min(matrix[row, positions[tag]], WEAK_CANDIDATE_WEIGHT)
...
def _label_coverage(records: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    coverage: dict[str, dict[str, int]] = {}
    for tag in ALLOWED_TAGS:
        candidate_count = sum(
            tag in set(filter_allowed_tags([
                *(record.get("raw_tags") or []),
                *(record.get("difficulty_tags") or []),
            ]))
            for record in records
        ...
        coverage[tag] = {
            ...
            "training_positive_count": sum(tag in (record.get(TRAINING_TARGET_FIELD) or []) for record in records),
            "validated_count": sum(tag in (record.get("validated_tags") or []) for record in records),
        }
    return coverage
...
def _assert_model_quality(
    ...
    holdout_recall = float(holdout_metrics.get("micro_recall", 0.0) or 0.0)
    if holdout_recall < MIN_HOLDOUT_RECALL:
        failures.append(f"留出集 recall={holdout_recall:.3f}")
    if coverage:
        missing_targets = [
            tag for tag, values in coverage.items()
            if values["candidate_count"] >= MIN_CANDIDATE_SUPPORT
            and values["training_positive_count"] < MIN_TARGET_POSITIVES
        ]
        if missing_targets:
            failures.append("候选标签未进入训练目标：" + "、".join(missing_targets))
    if failures:
...
def train_local_model(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    coverage = _label_coverage(records)
    if matrix.shape[1] == 0:
    ...
    _assert_model_quality(valid_metrics, holdout_metrics, coverage)
    label_metrics = []
    for index, tag in enumerate(ALLOWED_TAGS):
    ...
    loss_data = {
        ...
        "training_target_definition": "双模型/联网强监督标签加未被五标签展示槽位截断的XLS候选弱监督标签；最终展示标签只用于定数上限与单谱面展示",
        "label_coverage": coverage,
        "labels": ALLOWED_TAGS,
        "positive_counts": {tag: int(value) for tag, value in zip(ALLOWED_TAGS, targets.sum(axis=0))},
        "positive_rates": {
            tag: round(float(value) / max(len(records), 1), 6)
            for tag, value in zip(ALLOWED_TAGS, targets.sum(axis=0))
        },
        ...
        "quality_gate": {
            ...
            "minimum_holdout_recall": MIN_HOLDOUT_RECALL,
            "minimum_candidate_support": MIN_CANDIDATE_SUPPORT,
            "minimum_target_positives": MIN_TARGET_POSITIVES,
            "passed": True,
        },
    ...
    metadata = {
        ...
        "training_target_field": TRAINING_TARGET_FIELD,
        "training_target_definition": "双模型/联网强监督标签加未被五标签展示槽位截断的XLS候选弱监督标签；final_tags 仅为审计展示标签，不作为训练目标",
        "label_coverage": coverage,
        "labels": ALLOWED_TAGS,
        "target_positive_counts": {tag: int(value) for tag, value in zip(ALLOWED_TAGS, targets.sum(axis=0))},
        ...
        "supervision": "Uncapped XLS candidate weak targets plus dual-model consensus and traceable external evidence, grouped holdout, class weighting, early stopping and validation threshold calibration",
        "quality_gate": {
            "minimum_validation_precision": MIN_VALIDATION_PRECISION,
            "minimum_holdout_precision": MIN_HOLDOUT_PRECISION,
            "minimum_holdout_f1": MIN_HOLDOUT_F1,
...
def _training_target_usage(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    counter = Counter(tag for record in records for tag in _training_target_tags(record))
    total = max(len(records), 1)
    return [
        {"tag": tag, "count": int(counter.get(tag, 0)), "rate": round(counter.get(tag, 0) / total, 6)}
        for tag in ALLOWED_TAGS
...
def _select_training_targets(
    analysis: dict[str, Any],
    model_consensus: dict[str, Any],
    external_tags: list[str],
    validated_tags: list[str],
    candidate_validated_tags: list[str] | None = None,
    candidate_frequency: dict[str, int] | None = None,
) -> tuple[list[str], dict[str, float], dict[str, list[str]]]:
    """Build bounded multi-source targets without treating caps as negatives.

    Exact model agreement and externally grounded tags carry the most weight.
    XLS candidate/difficulty evidence remains learnable as weak supervision so
    a reviewer omission cannot erase a real local structure from the model.
    The five-label bound is applied after strong evidence is reserved; it is
    not allowed to make every record look like the same high-frequency label.
    """
    scores: dict[str, float] = {}
    sources: dict[str, list[str]] = {}

    def add(tags: Any, weight: float, source: str) -> None:
        for tag in filter_allowed_tags(tags or []):
            scores[tag] = max(scores.get(tag, 0.0), weight)
            sources.setdefault(tag, [])
            if source not in sources[tag]:
                sources[tag].append(source)

    model_tags = filter_allowed_tags(model_consensus.get("first_tags") or [])
    add(model_tags, 4.0, "dual_model_consensus")
    add(validated_tags, 4.5, "external_model_intersection")
    add(candidate_validated_tags or [], 4.25, "external_candidate_intersection")
    add(external_tags, 3.25, "external_source")

    difficulty_scores = analysis.get("difficulty_scores") if isinstance(analysis.get("difficulty_scores"), dict) else {}
    for tag in filter_allowed_tags(analysis.get("difficulty_tags") or []):
        try:
            strength = float(difficulty_scores.get(tag, 1.0) or 1.0)
        except (TypeError, ValueError):
            strength = 1.0
        scores[tag] = max(scores.get(tag, 0.0), 2.75 + min(1.25, strength))
        sources.setdefault(tag, [])
        if "xls_difficulty" not in sources[tag]:
            sources[tag].append("xls_difficulty")

    candidate_scores = analysis.get("candidate_scores") if isinstance(analysis.get("candidate_scores"), dict) else {}
    difficulty_scores = analysis.get("difficulty_scores") if isinstance(analysis.get("difficulty_scores"), dict) else {}
    for tag in filter_allowed_tags(analysis.get("raw_tags") or []):
        try:
            strength = max(
                float(candidate_scores.get(tag, 0.0) or 0.0),
                float(difficulty_scores.get(tag, 0.0) or 0.0),
            )
        except (TypeError, ValueError):
            strength = 0.0
        scores[tag] = max(scores.get(tag, 0.0), 2.0 + min(1.25, max(0.0, strength)))
        sources.setdefault(tag, [])
        if "xls_candidate" not in sources[tag]:
            sources[tag].append("xls_candidate")

    strong = set(filter_allowed_tags([
        *model_tags,
        *(validated_tags or []),
        *(candidate_validated_tags or []),
    ]))
    ...
    selected = strong_ranked[:MAX_FINAL_TAGS]
    candidate_pool = set(filter_allowed_tags([
        *(analysis.get("difficulty_tags") or []),
        *(analysis.get("raw_tags") or []),
        *external_tags,
    ])) - set(selected)
    frequency = candidate_frequency or {}
    total_frequency = max(sum(frequency.values()), 1)
    candidate_ranked = sorted(
        candidate_pool,
        key=lambda tag: (
            -(scores.get(tag, 0.0) + 0.75 * (1.0 - frequency.get(tag, 0) / total_frequency)),
            -len(sources.get(tag, [])),
            -tag_weight(tag),
            tag,
        ),
    )
    selected.extend(candidate_ranked[: max(0, MAX_FINAL_TAGS - len(selected))])
    return selected, {tag: round(scores[tag], 6) for tag in selected}, {tag: sources.get(tag, []) for tag in selected}


def _ensure_training_label_coverage(records: list[dict[str, Any]]) -> None:
    """Use auditable local candidates to fill otherwise lost label classes.

    The dual-model and network checks decide whether a record is eligible for
    the dataset.  This pass only resolves the independent five-label packing
    problem: a candidate with a concrete local window may replace a weaker
    unvalidated candidate, but never replaces a model or external consensus
    tag.  The chosen position and source are retained for auditability.
    """
    if not records:
        return
    for _ in range(len(ALLOWED_TAGS)):
        coverage = _label_coverage(records)
        missing = [
            tag for tag, values in coverage.items()
            if values["candidate_count"] >= MIN_CANDIDATE_SUPPORT
        ...
        if not missing:
            return
        changed = False
        for tag in sorted(missing, key=lambda value: (coverage[value]["training_positive_count"], value)):
            eligible: list[tuple[float, str, dict[str, Any]]] = []
            for record in records:
                candidates = set(filter_allowed_tags([
                    *(record.get("raw_tags") or []),
                    *(record.get("difficulty_tags") or []),
                ]))
                evidence = record.get("tag_evidence") if isinstance(record.get("tag_evidence"), dict) else {}
                if tag not in candidates or not evidence.get(tag):
                    continue
                scores = record.get("candidate_scores") if isinstance(record.get("candidate_scores"), dict) else {}
                difficulty_scores = record.get("difficulty_scores") if isinstance(record.get("difficulty_scores"), dict) else {}
                score = max(
                    float(scores.get(tag, 0.0) or 0.0),
                    float(difficulty_scores.get(tag, 0.0) or 0.0),
                )
                eligible.append((score, str(record.get("record_key", "")), record))
            eligible.sort(key=lambda item: (-item[0], item[1]))
            for _score, _key, record in eligible:
                tags = list(filter_allowed_tags(record.get(TRAINING_TARGET_FIELD) or []))
                if tag in tags:
                    continue
                validation = record.get("validation") if isinstance(record.get("validation"), dict) else {}
                protected = set(filter_allowed_tags([
                    *((record.get("model_consensus") or {}).get("first_tags") or []),
                    *(validation.get("intersection_tags") or []),
                    *(validation.get("candidate_intersection_tags") or []),
                ]))
                was_full = len(tags) >= MAX_FINAL_TAGS
                # Preserve every label that is already at the minimum global
                # coverage. Otherwise filling a rare label can silently
                # remove the only other examples of a different rare label.
                removable = [
                    value for value in tags
                    if value not in protected
                    and coverage.get(value, {}).get("training_positive_count", 0) > MIN_TARGET_POSITIVES
                ]
                removed_tag: str | None = None
                if len(tags) < MAX_FINAL_TAGS:
                    tags.append(tag)
                elif removable:
                    scores = record.get("training_tag_scores") if isinstance(record.get("training_tag_scores"), dict) else {}
                    removed_tag = min(removable, key=lambda value: (float(scores.get(value, 0.0) or 0.0), value))
                    tags[tags.index(removed_tag)] = tag
                else:
                    continue
                record[TRAINING_TARGET_FIELD] = filter_allowed_tags(tags)[:MAX_FINAL_TAGS]
                training_scores = record.get("training_tag_scores") if isinstance(record.get("training_tag_scores"), dict) else {}
                candidate_scores = record.get("candidate_scores") if isinstance(record.get("candidate_scores"), dict) else {}
                difficulty_scores = record.get("difficulty_scores") if isinstance(record.get("difficulty_scores"), dict) else {}
                training_scores[tag] = round(max(
                    float(candidate_scores.get(tag, 0.0) or 0.0),
                    float(difficulty_scores.get(tag, 0.0) or 0.0),
                    tag_weight(tag),
                ), 6)
                record["training_tag_scores"] = training_scores
                training_sources = record.get("training_tag_sources") if isinstance(record.get("training_tag_sources"), dict) else {}
                training_sources[tag] = sorted(set((training_sources.get(tag) or []) + ["coverage_balancing", "xls_local_window"]))
                record["training_tag_sources"] = training_sources
                changed = True
                coverage.setdefault(tag, {"candidate_count": 0, "training_positive_count": 0, "validated_count": 0})
                coverage[tag]["training_positive_count"] += 1
                if was_full and removed_tag is not None:
                    coverage[removed_tag]["training_positive_count"] = max(
                        0,
                        coverage.get(removed_tag, {}).get("training_positive_count", 0) - 1,
                    )
                if sum(tag in (item.get(TRAINING_TARGET_FIELD) or []) for item in records) >= MIN_TARGET_POSITIVES:
                    break
        if not changed:
            return
...
def _build_training_records(
    ...
    for item in selected:
        ...
        candidate_frequency.update(filter_allowed_tags([
            *(analysis.get("difficulty_tags") or []),
            *(analysis.get("raw_tags") or []),
        ]))
    ...
    for index, item in enumerate(selected, start=1):
        ...
        baseline = filter_allowed_tags(
            model_consensus.get("first_tags")
            or analysis.get("difficulty_tags")
        ...
        candidate_validated_tags = filter_allowed_tags(
            validation.get("candidate_intersection_tags") or []
        )
        training_tags, training_scores, training_sources = _select_training_targets(
            analysis,
            model_consensus,
            external_tags,
            validated_tags,
            candidate_validated_tags,
            dict(candidate_frequency),
        )
        record.update({
            "record_version": 9,
            ...
            "training_tag_scores": training_scores,
            "training_tag_sources": training_sources,
            "training_label_source": "dual_model_gate_external_evidence_xls_candidates_with_coverage_balancing",
    ...
    # A second pass closes gaps created when an earlier rare label occupied
    # the only replaceable slot in a record.
    _ensure_training_label_coverage(records)
...
def build_report(records: list[dict[str, Any]], training: dict[str, Any], selection: dict[str, Any]) -> str:
    ...
    target_map = {row["tag"]: row for row in _training_target_usage(records)}
    sampled_map = {row["tag"]: row for row in _usage(records, TRAINING_TARGET_FIELD)}
    for tag in ALLOWED_TAGS:
        lines.append(f"| {tag} | {raw_map[tag]['count']} ({raw_map[tag]['rate']:.2%}) | {model_map[tag]['count']} ({model_map[tag]['rate']:.2%}) | {external_map[tag]['count']} ({external_map[tag]['rate']:.2%}) | {validated_map[tag]['count']} ({validated_map[tag]['rate']:.2%}) | {target_map[tag]['count']} ({target_map[tag]['rate']:.2%}) | {sampled_map[tag]['count']} ({sampled_map[tag]['rate']:.2%}) | {final_map[tag]['count']} ({final_map[tag]['rate']:.2%}) |")
    lines.extend([
        "",
        "## 逐谱面有效样本标注",
        "",
    ...
    for index, record in enumerate(records, start=1):
        ...
        lines.append(
            ...
            f"{'、'.join(filter_allowed_tags([*(record.get('raw_tags') or []), *(record.get('difficulty_tags') or [])])) or '无'} | "
