...
EXTERNAL_REVIEW_RETRY_DELAYS = (5, 15, 30, 60, 120)
...
def _external_review_key() -> str:
    return str(
        os.environ.get(EXTERNAL_REVIEW_KEY_ENV)
        or os.environ.get("STARMIAOA_API_KEY")
        or ""
    ).strip()
...
def _review_external_tags(ref: dict[str, Any], baseline: list[str], sources: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for attempt, delay in enumerate((0, *EXTERNAL_REVIEW_RETRY_DELAYS)):
        if delay:
            time.sleep(delay)
        try:
            ...
            if exc.code not in {408, 425, 429, 500, 502, 503, 504} or attempt >= len(EXTERNAL_REVIEW_RETRY_DELAYS):
                return {"status": "unavailable", "tags": [], "evidence": [], "reason": f"HTTPError: {exc.code}"}
        except (TimeoutError, urllib.error.URLError, OSError) as exc:
            if attempt >= len(EXTERNAL_REVIEW_RETRY_DELAYS):
                return {"status": "unavailable", "tags": [], "evidence": [], "reason": f"{type(exc).__name__}: {exc}"[:240]}
        except Exception as exc:
            return {"status": "unavailable", "tags": [], "evidence": [], "reason": f"{type(exc).__name__}: {exc}"[:240]}
    return {"status": "unavailable", "tags": [], "evidence": [], "reason": "retry_exhausted"}
...
def collect_external_evidence(ref: dict[str, Any]) -> dict[str, Any]:
    ...
    model_tags = filter_allowed_tags(ref.get("model_tags") or ref.get("consensus_tags") or [])
    candidate_tags = filter_allowed_tags(ref.get("candidate_tags") or [])
    review_tags = filter_allowed_tags([*model_tags, *candidate_tags])
    ...
    try:
        ...
        model_review = _review_external_tags(ref, review_tags, sources)
        result["keyword_tags"] = keyword_tags
        result["external_model_review"] = model_review
        if _external_review_key():
...
def select_effective_samples(
    ...
    *,
    ...
    def review_one(entry: tuple[int, dict[str, Any], dict[str, Any], dict[str, Any]]) -> dict[str, Any]:
        ...
        model_baseline = filter_allowed_tags(model_consensus.get("first_tags") or [])
        candidate_tags = filter_allowed_tags([
            *(analysis.get("difficulty_tags") or []),
            *(analysis.get("raw_tags") or []),
        ])
        review_tags = filter_allowed_tags([*model_baseline, *candidate_tags])
        cache_key = str(ref.get("key", ""))
        evidence = _cached_external_evidence(cache_key)
        if evidence is None:
            evidence = collect_external_evidence({
                **ref,
                "model_tags": model_baseline,
                "candidate_tags": candidate_tags,
            })
            _cache_external_evidence(cache_key, evidence)
        external = filter_allowed_tags(evidence.get("external_tags") or [])
        overlap, intersection, symmetric_jaccard = _overlap(model_baseline, external)
        candidate_intersection = sorted(set(candidate_tags) & set(external))
        return {
            "index": index,
            "item": {
                "ref": ref,
                ...
                "validation": {
                    "baseline_tags": model_baseline,
                    "candidate_tags": candidate_tags,
                    "review_tags": review_tags,
                    "external_tags": external,
                    "intersection_tags": intersection,
                    "candidate_intersection_tags": candidate_intersection,
                    "overlap": round(overlap, 6),
                    "symmetric_jaccard": round(symmetric_jaccard, 6),
                    "threshold": EFFECTIVE_OVERLAP,
                    "effective": bool(evidence.get("sources") and model_baseline and overlap >= EFFECTIVE_OVERLAP),
    ...
    for start in range(0, len(agreed), EXTERNAL_REVIEW_BATCH_SIZE):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=worker_count) as pool:
            ...
            for future in futures:
                ...
                validation["training_eligible"] = bool(
                    sources
                    and (
                        validation.get("baseline_tags")
                        or validation.get("candidate_tags")
                        or validation.get("external_tags")
                    )
                )
    ...
    if len(strict) >= target:
        strict = _select_coverage_balanced(strict, target)
        for item in strict:
    ...
    selected = _select_coverage_balanced(fallback, FALLBACK_SAMPLE_TARGET)
...
def _select_coverage_balanced(items: list[dict[str, Any]], target: int) -> list[dict[str, Any]]:
    """Prefer reviewed charts that add underrepresented candidate labels.

    The model-consensus and evidence gates have already run before this
    function.  This only decides which gated records to retain, using the
    local candidate set as a coverage signal so common configurations do not
    consume the whole metadata budget.
    """
    if len(items) <= target:
        return list(items)
    ...
    while remaining and len(chosen) < target:
        def score(item: dict[str, Any]) -> tuple[float, float, int, int]:
            validation = item.get("validation") if isinstance(item.get("validation"), dict) else {}
            labels = set(
                filter_allowed_tags([
                    *(validation.get("candidate_tags") or []),
                    *(validation.get("baseline_tags") or []),
                    *(validation.get("external_tags") or []),
                ])
            )
            coverage_gain = sum(1.0 / (1.0 + counts.get(tag, 0)) for tag in labels)
            confidence = float(validation.get("confidence", 0.0) or 0.0)
            source_count = int(validation.get("evidence_source_count", 0) or 0)
            index = int(item.get("index", 0) or 0)
            return coverage_gain, confidence, source_count, -index

        best = max(enumerate(remaining), key=lambda pair: score(pair[1]))
        item = remaining.pop(best[0])
        chosen.append(item)
        validation = item.get("validation") if isinstance(item.get("validation"), dict) else {}
        labels = filter_allowed_tags([
            *(validation.get("candidate_tags") or []),
            *(validation.get("baseline_tags") or []),
            *(validation.get("external_tags") or []),
        ])
