...
"""Runtime chart lookup and background download/analysis tasks.

Chart annotations are deliberately process-local.  The only durable chart
artifacts are the source files in Levels and the audited training dataset;
normal tag lookup always reparses the requested difficulty and runs the local
model.
"""
...
class LocalChartModel:
    ...
    def _load(self) -> None:
        if not self.model_file.is_file() or not self.metadata_file.is_file():
            raise FileNotFoundError("本地谱面标签模型文件不存在，请先准备训练模型")
        model_stat = self.model_file.stat()
        meta_stat = self.metadata_file.stat()
        marker = (int(model_stat.st_mtime_ns), int(model_stat.st_size), int(meta_stat.st_mtime_ns))
        if self._mtime == marker and self.weights is not None:
            return
        metadata = json.loads(self.metadata_file.read_text(encoding="utf-8"))
        if not isinstance(metadata, dict) or metadata.get("analysis_engine") != RULE_ENGINE:
            raise ValueError("模型不是当前本地规则引擎训练的模型")
        if int(metadata.get("rule_version", -1) or -1) != TAG_RULE_VERSION:
            raise ValueError("模型规则版本与当前标签规则不一致")
    ...
    def predict(self, song: MaidataSong, chart: MaidataChart) -> dict[str, Any]:
        ...
        return {
            ...
            "model_metadata": {
                ...
                "analysis_engine": self.metadata.get("analysis_engine", ""),
                "rule_version": self.metadata.get("rule_version"),
...
class LocalChartCatalog:
    """Cache only file metadata; chart annotations are never persisted."""
...
def _mapping_from_ref(ref: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "chart_info": {
            key: ref.get(key)
            for key in ("title", "artist", "genre", "version", "level", "ds", "bpm", "whole_bpm", "designer")
        },
        "tag_source": "local_chart_tag_model",
    }
...
class AutoTagJob:
    """Coordinate background work while keeping annotations in memory only."""
    ...
    def status(self) -> dict[str, Any]:
        ...
        state.update({
            ...
            "catalog_downloaded_total": len(refs),
            "catalog_analyzed": len(analyzed_keys),
            "catalog_tagged": tagged,
            "catalog_untagged": max(0, len(eligible) - len(analyzed_keys)),
            "model_file": _relative_path(MODEL_FILE),
            "model_metadata": self.model.metadata if not model_error else {},
    ...
    def start_download(self, *, min_ds: Any, max_ds: Any, mode: Any, query: Any = "") -> dict[str, Any]:
        ...
        with self.lock:
            if self.worker_thread and self.worker_thread.is_alive():
                return {"ok": True, "message": "已有谱面任务在运行", **self.status()}
            self.stop_requested = False
            self._write_state({
                "task": "download",
                "status": "running",
    ...
    def stop(self) -> dict[str, Any]:
        ...
        return {"ok": True, **self._write_state({"stop_requested": True, "message": "已请求停止，当前谱面处理完成后停止"}), **self.status()}
    ...
    def shutdown(self) -> None:
        ...
        self._write_state({"status": "stopped", "running": False, "message": "谱面任务已停止"})
    ...
    def _run_download(self, low: float, high: float, mode: str, query: str) -> None:
        try:
            ...
            self._progress({"task": "download", "status": "failed", "error": f"{type(exc).__name__}: {exc}", "finished_at": now_text(), "current": ""})
        finally:
            self.worker_thread = None
    ...
    def _analyze_ref(self, ref: dict[str, Any]) -> dict[str, Any]:
        ...
        item.update({
            ...
            "tag_categories": {tag: TAG_CATEGORIES.get(tag, "") for tag in final_tags},
    ...
    def _failed_item(self, ref: dict[str, Any], exc: Exception) -> dict[str, Any]:
        item = _base_item(ref)
        item.update({
    ...
    def _run_analysis(self, low: float, high: float, force: bool) -> None:
        try:
            refs = [
                ref for ref in self.catalog.refs()
                if max(low, MIN_TAG_DS) <= float(ref["ds"]) <= high
            ]
            self._progress({
                "task": "analysis",
                "status": "running",
            ...
            self._progress({"task": "analysis", "status": "failed", "error": f"{type(exc).__name__}: {exc}", "finished_at": now_text(), "current": ""})
        finally:
            self.worker_thread = None
    ...
    def detail(self, key: str) -> dict[str, Any] | None:
        ...
        file_mapping = {
            ...
            "chart_sections": [
                {
                    "mapping_id": f"{other['song_id']}:{other['diff_id']}",
                    "diff_id": other["diff_id"],
                    "level_index": other["level_index"],
                    "chart_section": f"inote_{other['diff_id']}",
                    "tag_key": other["key"],
                }
                for other in self.catalog.refs()
                if other.get("source_path") == ref.get("source_path")
            ],
        }
        return _detail(item, key, file_mapping)


def _summary(item: dict[str, Any], key: str) -> dict[str, Any]:
    model_tags = filter_allowed_tags(item.get("model_tags") or [])
    final_tags = filter_allowed_tags(item.get("final_tags") or item.get("tags") or model_tags)
